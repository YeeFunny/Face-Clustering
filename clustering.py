...
def cluster_facial_encodings(facial_encodings, threshold, iteration):
    ...
    sorted_clusters = _chinese_whispers(facial_encodings.items(), threshold, iteration)
    return sorted_clusters
...
def get_onedir(paths):
    ...
    Check if dir exists and get images in a directory
...
def main(args):
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            image_paths = get_onedir(args.input_dir)
            ...
            sorted_clusters = cluster_facial_encodings(facial_encodings, args.threshold, args.iteration)
            num_cluster = len(sorted_clusters)
                
            # Copy image files to cluster folders
            for idx, cluster in enumerate(sorted_clusters):
                cluster_dir = join(args.output_dir, str(idx))
...
def parse_args():
    ...
    parser.add_argument('--threshold', type=float, help='Value to cluster two faces', required=True)
    parser.add_argument('--iteration', type=int, help='The number of iterations', required=True)
    parser.add_argument('--input_dir', type=str, help='Input dir of images', required=True)
    parser.add_argument('--output_dir', type=str, help='Output dir of clusters', required=True)
